...
With `FLUXMAIL_AUTH=apikey`, each HTTP API key can have its own permission profile. The profile applies to MCP and REST. `FLUXMAIL_AUTH=none` gives every HTTP client full access, so use it only behind a network boundary you control. See [Limit what a connection can do](/docs/permissions) for profiles and custom policies.
